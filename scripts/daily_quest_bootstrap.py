...
def recognize_bioenhancer(frame_path: Path) -> Dict[str, Any]:
    """Recognize Bioenhancer research screen identity without authorizing research."""
    frame = frame_from(frame_path)
    title = ocr(frame, ROIS["bioenhancer_title"], psm=6)
    content = ocr(frame, ROIS["bioenhancer_content"], psm=6)
    combined = f"{title} {content}"
    recognized = ocr_has_phrase(combined, "bioenhancer", threshold=0.80) and ocr_has_phrase(
