...
- Established completion evidence: offline focused 19 + regressions 232 + full suite 871 passed /
  ...
  `CONFIRMED_NOT_DISPATCHED=NON_DISPATCH_AUTHORITY_UNAVAILABLE`.
- Next: `SUPPLY-DEPOT-VERIFIED-ROUTE-INTEGRATION` (completed as `f523f0f`); active seam closure
  `HOME-ATLAS-VERIFIED-ROUTE-SEAM-CLOSURE`; `RUNTIME-DECLARATIVE-VERIFIED-FLOW-COMPOSITION`
  remains blocked until renewed readiness passes after Home Atlas and Supply Depot seam closures.

### HOME-ATLAS-VERIFIED-ROUTE-SEAM-CLOSURE
- Task ID: `HOME-ATLAS-VERIFIED-ROUTE-SEAM-CLOSURE`.
- Title: Close Home Atlas verified-route readiness seams (fresh pre_dispatch + action-ledger parity).
- Status: Completed (2026-07-19; offline review + bounded live BlueStacks reversible navigate-building
  regression passed; local commit `fix(navigation): close home atlas verified route seams`; no push).
- Milestone: Durable offline navigation architecture roadmap; composition-readiness prerequisite.
- Dependencies: completed `HOME-ATLAS-VERIFIED-ROUTE-INTEGRATION` (`f093812`), renewed readiness
  review recorded in `docs/navigation_verified_flow_readiness.md` (`9d814d3`).
- Blocked by: None remaining for this task. Do not modify Supply Depot. Do not activate composition or M6.
- Objective: close only the two Home Atlas findings from the renewed readiness review:
  (1) replace cached issuance-observation reuse in `dispatch_verified_navigate_pan` executor
  `recapture()` with a genuinely fresh immediate pre_dispatch frame acquisition and semantic
  rebind that fails closed on drift; final capability must be issued against that fresh
  pre_dispatch frame; (2) bring navigate-building to full shared action-ledger parity with
  distinct prepared/authorized/dispatched/transport-observed/semantically-verified/completed/
  failed/unresolved states as defined by existing contracts.
- Established facts: navigate-building already uses NavigationSession, perception bundle,
  calibration, observability, CentralPolicy, SafeActionExecutor, and seal-gated pan transport;
  readiness still records cached `recapture()` and incomplete six-state ledger parity;
  `CONFIRMED_NOT_DISPATCHED` remains `NON_DISPATCH_AUTHORITY_UNAVAILABLE`.
- Direct implementation files: `scripts/home_atlas_bluestacks.py` (Home Atlas navigate-building /
  `dispatch_verified_navigate_pan` only; do not modify Supply Depot helpers),
  `tests/test_home_atlas_verified_route.py`, narrowly required shared adoption touches only when
  required (`safe_action_core/*`, `tasks/navigation_session.py`, `tasks/navigation_observability.py`,
  `tasks/perception_bundle.py`), `BACKLOG.md`, `CURRENT_HANDOFF.md`, and
  `tests/test_governance_validation.py` only if durable identity coverage changes.
- Shared dependencies: existing Home Atlas planner/localizer/runtime adapters, CentralPolicy,
  SafeActionExecutor, NavigationSession, observability, session calibration, perception bundle;
  no production registration.
- Transitive regression set: home-atlas verified-route, navigation session/observability/
  calibration, capability/firewall, and governance focused suites.
- Allowed changes: per-commit allowed paths are exactly the direct implementation files above;
  no atlas expansion; no new destinations; no Supply Depot edits.
- Prohibited changes: Supply Depot route/helpers, new destinations, atlas rebuild/expansion,
  second engine/DSL, enabling `CONFIRMED_NOT_DISPATCHED`, consequential gameplay, registration,
  scheduler, workers, force-push, amend, rebase, reset/clean, or push.
- Authorized runtime action: after offline review passes, exactly one bounded live BlueStacks
  Home Atlas regression using only reversible navigation already owned by the route (bounded
  camera pans, supported menu/facility navigation, overlay dismissal, verified return toward
  Home). Zero consequential actions.
- Maximum transport inputs: offline phase zero; live phase only the minimum navigation-only inputs
  required to prove fresh pre_dispatch capability binding and ledger reconciliation.
- Navigation-only recovery: bounded recovery already owned by the navigate route may run; no
  identical retries without a concrete new hypothesis.
- Consequential action: None; claims, purchases, premium, training, healing, research, upgrades,
  crafting, resource consumption, and troop deployment remain forbidden.
- Registration changes: None; production remains `NOT_REGISTERED`.
- Scheduler changes: None; scheduler remains disabled/ineligible.
...
- Required source: current immutable native 800x1280 frames from the existing BlueStacks Home Atlas
  navigate route; offline tests may use fixtures/mocks without live capture.
- Exact target semantics: positively recognized Home/base checkpoint; fresh pre_dispatch capture
  with semantic rebind; capability issued against that fresh frame; executor-only dispatch;
  distinct transport-observed vs semantically verified/completed; full action-ledger parity.
- Required local association: every authorized pan cites navigation session id, action id/key,
  target identity/ROI, fresh capture digest+monotonic, and capability audit; observability and
  optional calibration reports attach without mutating authority.
- Negative controls: returning the issuance Observation object from `recapture()` without a new
  capture, capability reuse/forgery, stale/cross-capture dispatch, treating transport success as
  semantic success, Supply Depot edits, atlas expansion, and consequential controls.
- Coordinate space: BlueStacks native 800x1280 full-frame evidence only; no scaled previews or Bliss
  coordinate reuse.
- Accepted signals: offline focused/adversarial/governance/full-suite gates; live reversible Home
  navigation proving capability issued against fresh pre_dispatch frame, SafeActionExecutor-only
  dispatch, and ledger reconciliation with transport and semantic verification.
- Rejected weak signals: imports-only or metadata-only adoption, dry-run-only live claims, and
  transport success without successor verification.
- Ambiguous-result behavior: fail closed to unresolved/blocked; never blind-retry identical input.
- Zero-cost requirement: navigation-only camera pans and reversible Home navigation; no resource or
...
- Active evidence manifest: None for offline work; live session artifacts remain under
  `.local-captures` and must not stage protected `evidence/**`.
- Required artifacts: updated `dispatch_verified_navigate_pan` with fresh pre_dispatch capture/
  rebind; navigate-building result/action ledger parity; focused offline tests including adversarial
  stale/cross-capture/cached-recapture probes; live navigation-only session result retained locally
...
- Focused tests: `tests.test_home_atlas_verified_route` plus touched home-atlas session/observability/
  calibration/capability regressions.
- Integration tests: offline mocked fresh-pre_dispatch capability-bound navigate path; after offline
  pass, one bounded live reversible Home navigation regression.
- Transitive regression tests: governance validation and focused discovery of touched modules.
- Full-suite requirement: run focused tests first, then touched-component regressions, adversarial
  probes, then the full repository suite before live validation and before commit.
- Validators: Python compilation, focused tests, governance validation, CURRENT_HANDOFF JSON
...
- Valid blocked outcomes: inability to implement fresh pre_dispatch without weakening bindings,
  ledger parity failure, live consequential pressure, or offline suite regressions.
- Blocked-result commit policy: record the blocker; do not commit a partial seam-preserving route
  as complete; preserve WIP and evidence.
- Commit policy: one reviewed conventional local commit after both offline and live validation pass;
  no push.
- Expected focused commits: `fix(navigation): close home atlas verified route seams`; allowed paths
  are the direct implementation files above.
- Completion criteria: Home Atlas navigate-building uses genuine fresh pre_dispatch capture/rebind
  with capability issued against that frame; full action-ledger parity; offline gates pass; one
  bounded live reversible validation passes; Supply Depot untouched; registration/scheduler
  unchanged; `CONFIRMED_NOT_DISPATCHED` still unavailable; composition remains blocked; commit
  created locally without push.
- Established completion evidence: offline focused 22 + regressions 153 + full suite 891 passed /
  1 skipped; live Bank navigate 1 pan with full action_ledger and planning digest ≠
  pre_dispatch_frame_sha256; live HQ return 2 pans each with planning≠pre_dispatch and
  requested/authorized/dispatched/transport_observed/verified/completed; artifacts under
  `.local-captures/home-atlas-seam-closure/`; Supply Depot untouched; registration/scheduler
  unchanged; `CONFIRMED_NOT_DISPATCHED=NON_DISPATCH_AUTHORITY_UNAVAILABLE`.
- Next: leave `RUNTIME-DECLARATIVE-VERIFIED-FLOW-COMPOSITION` blocked until Supply Depot seams are
  also closed and readiness is renewed; leave `M6-DQ-TRANSITION-CORPUS` unactivated.
...
- Dependencies: completed `SUPPLY-DEPOT-VERIFIED-ROUTE-SEAM-CLOSURE` (`437a52c`), completed
  ...
  `docs/navigation_verified_flow_readiness.md`, and completed
  `SUPPLY-DEPOT-RADIAL-TARGET-BINDING-CLOSURE` before renewal.
- Blocked by: pending renewal under corrected Claim Supply pairing from completed
  `SUPPLY-DEPOT-RADIAL-TARGET-BINDING-CLOSURE` (do not retry identical pre-fix ROI
  `[555,551,725,657]`). Do not implement composition. Do not activate M6. Do not claim supplies.
...
- Task ID: `SUPPLY-DEPOT-RADIAL-TARGET-BINDING-CLOSURE`.
- Title: Close Supply Depot radial Claim Supply target-binding seam that selected Upgrade.
- Status: Completed (2026-07-20; offline root-cause closure: Claim+Supply OCR proximity
  pairing rejects building-title `Sup` contamination that produced Upgrade-covering
  `[555,551,725,657]`; local commit
  `fix(navigation): close supply depot radial target binding seam`; no push; no live retry).
- Milestone: Durable offline navigation architecture roadmap; composition-readiness prerequisite.
- Dependencies: blocked `SUPPLY-DEPOT-VERIFIED-ROUTE-LIVE-BINDER-EVIDENCE` (attempt 1 ROI
  `[555,551,725,657]` opened Upgrade; attempt 2 `RADIAL_REBIND_FAILED`), completed
  `SUPPLY-DEPOT-VERIFIED-ROUTE-SEAM-CLOSURE` (`437a52c`), retained live frames under
  `.local-captures/supply-depot-live-binder-evidence/`.
- Blocked by: None remaining for this task once activated. Do not modify safe-exit binder behavior.
  Do not implement composition. Do not activate M6. Do not claim supplies. Do not repeat identical
  failed live ROI attempts.
- Objective: make Supply Depot radial Claim Supply target binding deterministic enough that the
  existing facility safe-exit binder path can later be live-validated. Trace
  `command_supply_depot_radial` → building entry → radial acquisition → radial pre-dispatch →
  Claim Supply binding → `SafeActionExecutor`; correct the narrow perception/binding defect that
  caused ROI `[555,551,725,657]` to hit Upgrade; preserve fail-closed behavior.
- Established facts: attempt-1 live OCR showed building-title `Sup` plus radial `Claim`/`Suppl`;
  `_claim_supply_roi_from_data` unioned all `clai*`/`sup*` tokens into Upgrade-covering
  `[555,551,725,657]`; attempt 2 fail-closed without Details OCR (`RADIAL_REBIND_FAILED`); safe-exit
  packaging already emits `home_safe_exit_probe` when exit is reached; this task is not a claim or
  safe-exit rewrite.
- Direct implementation files: `tasks/supply_depot_vision.py`,
  `tests/test_supply_depot_vision.py`, `tests/test_supply_depot_verified_route.py` only if route
  regressions require it, `BACKLOG.md`, `CURRENT_HANDOFF.md`,
  `docs/navigation_verified_flow_readiness.md` only if blocker status materially changes, and
  `tests/test_governance_validation.py` for durable identity coverage.
- Shared dependencies: existing Supply Depot verified route, radial semantics, perception bundle,
  capability firewall, `SafeActionExecutor`; no production registration.
- Transitive regression set: supply-depot vision/verified-route, radial semantics, perception
  bundle, capability firewall, SafeActionExecutor, governance.
- Allowed changes: per-commit allowed paths are exactly the direct implementation files above.
- Prohibited changes: safe-exit binder behavior, facility exit logic, new navigation engines,
  new perception models, LLM/runtime inference, weakening capability validation, bypassing
  `SafeActionExecutor`, fixed-coordinate authorization, direct transport, gameplay/claims,
  composition/M6 activation, registration/scheduler changes, enabling
  `CONFIRMED_NOT_DISPATCHED`, force-push, amend, rebase, reset/clean, or push.
- Authorized runtime action: None required for offline binding closure. Live validation is
  permitted only if the exact failure cause is identified, the correction is bounded, and no
  unchanged failed attempt is repeated; otherwise leave live to a renewed LIVE-BINDER task.
- Maximum transport inputs: Zero unless a separately justified bounded live proof is required
  after offline gates; default is offline-only.
- Navigation-only recovery: Home dismissal uses supported non-consequential map deselection only;
  do not use Android Back on Home map.
- Consequential action: None; claims, purchases, premium, training, healing, research, upgrades,
  crafting, resource consumption, and troop deployment remain forbidden.
- Registration changes: None; production remains `NOT_REGISTERED`.
- Scheduler changes: None; scheduler remains disabled/ineligible.
...
- Required source: retained live attempt-1/2 radial pre-dispatch frames plus offline OCR/box
  fixtures proving building-label contamination versus paired Claim Supply geometry.
- Exact target semantics: Claim Supply ROI binds only co-located Claim+Supply OCR tokens; Upgrade
  and building-title Supply/Sup cannot satisfy the binding; fresh pre-dispatch geometry remains
  authoritative; capability issuance still requires valid semantic target binding.
- Required local association: binding cites current-frame OCR boxes and fail-closed pairing;
  capability path retains fresh capture digest + rebound ROI.
- Negative controls: union-all `sup*` contamination; Upgrade-as-Claim-Supply; fixed-ROI independent
  authorization; claims; composition activation.
- Coordinate space: BlueStacks native 800x1280 full-frame evidence only.
- Accepted signals: offline proof that live attempt-1 contamination ROI is rejected, paired Claim
  Supply ROI excludes Upgrade, and capability/route regressions remain green.
- Rejected weak signals: coordinate-only guesses without OCR contamination proof; transport success
  as semantic proof.
- Ambiguous-result behavior: fail closed; never blind-retry identical input.
- Zero-cost requirement: navigation-only; no resource or premium spend.
- Quantity limits: one atomic binding closure; zero supply claims.
- Resource consumption policy: no game resources consumed.
- Premium or strategic restrictions: no premium/purchase/strategic controls.
- Active evidence manifest: None; retain prior live artifacts under `.local-captures` without
  staging protected `evidence/**`.
- Required artifacts: corrected Claim Supply ROI pairing; focused adversarial tests; updated
  backlog/handoff (and readiness note only if blocker status changes).
- Immediate-before/immediate-post/result/journal: NOT_APPLICABLE for offline-only closure.
- Additional task-specific artifacts: none beyond code/tests/docs listed above.
- Focused tests: `tests.test_supply_depot_vision`, `tests.test_supply_depot_verified_route`, radial
  semantics, perception bundle, capability firewall, SafeActionExecutor, governance.
- Integration tests: offline mocked Supply Depot verified route remains green.
- Transitive regression tests: governance validation and focused discovery of touched modules.
- Full-suite requirement: run focused tests first, then touched regressions, then the full
  repository suite before commit. Expected baseline: 900 passed, 1 skipped (count may rise with
  new focused cases).
- Validators: Python compilation for any touched Python files, focused tests, governance
  validation, CURRENT_HANDOFF JSON parsing, touched-file secret scan, and `git diff --check`.
- Known baseline failures: None; one expected full-suite skip may remain.
- Evidence requirement: NOT_APPLICABLE because this task corrects offline binding using retained
  local captures without promoting into protected `evidence/**`.
- Valid blocked outcomes: inability to prove contamination root cause; pressure to weaken
  capability/safe-exit; live consequential pressure; offline suite regressions.
- Blocked-result commit policy: if setup-only, record the exact setup correction without production
  code change; do not claim composition readiness.
- Commit policy: one reviewed conventional local commit; no push. Prefer
  `fix(navigation): close supply depot radial target binding seam` when production changes; else
  `docs(navigation): record supply depot radial binding blocker`.
- Expected focused commits: exactly one of the commit messages above; allowed paths are the direct
  implementation files above.
- Completion criteria: root cause identified; smallest atomic correction (or setup-only record)
  applied; focused/adversarial tests prove Upgrade and building-label Supply cannot satisfy Claim
...
- Status: Completed (2026-07-24); session `survey-20260724T023336884972Z` reached verified `native_survey_complete` with recognized Home; independent evidence review accepted the retained chain; 93 of 272 inputs were used (AUX7/EDGE24/OVERLAP62).
...
- Blocked by: none. Atlas integration and consumer promotion remain separate downstream tasks.
...
- Established facts: the reviewed budget is 272 and inputs used are 93 = AUX7/EDGE24/OVERLAP62 from accepted evidence ending at survey-20260724T023336884972Z (native_survey_complete; recognized Home; one exit input this session); remaining 179 total / AUX9 / EDGE104 / OVERLAP66; zero-input VIP popup-block survey-20260724T002912186392Z retained uncounted; selection-aware tier templates and unhighlighted exit asset/manifest are checked in as task ground truth.
- Direct implementation files: `tasks/campaign_atlas.py`, `tasks/campaign_atlas_vision.py`, `scripts/campaign_atlas_bluestacks.py`, `scripts/flow_delivery_campaign_atlas_bluestacks.py`, `scripts/home_atlas_bluestacks.py` (narrow shared Campaign-entry lifecycle correction only), `scripts/pnsctl.py`, `safe_action_core/policy.py` and `scripts/personal_might_praise_live.py` (task-scoped VIP popup correction only), `tasks/assets/campaign_auto_battle/800x1280/` selection-aware tier and unhighlighted exit templates/ground-truth/manifest, `tasks/flow_delivery_bluestacks_registry.json`, the Campaign survey gameplay contract, and dedicated Campaign atlas/authority plus Campaign-entry verified-route tests.
...
- Allowed changes: per-commit allowed paths are the direct implementation files (including selection-aware tier/exit assets and task-scoped popup policy/personal-might paths), `tasks/flow_delivery_queue.json`, `tasks/backlog_task_index.json`, `tasks/gameplay_flow_contracts/CAMPAIGN-ATLAS-NATIVE-SURVEY-AND-VALIDATION.json`, dedicated Campaign atlas tests, `tests/test_home_atlas_verified_route.py`, `tests/test_personal_might_praise.py`, directly attributable flow-delivery authority/registry tests, this exact backlog section, and `CURRENT_HANDOFF.md`.
...
- Authorized runtime action: One pnsctl-only navigation survey session using HUD-safe pans and current-frame measured Story difficulty 1/2 comparison taps; no gameplay execution.
- Maximum transport inputs: 272 navigation-only inputs, with 93 used (AUX7/EDGE24/OVERLAP62 after exit completion); remaining 179.
...
- Quantity limits: exactly one live survey session with a hard ceiling of 272 navigation inputs; cumulative inputs used are 93.
- Resource consumption policy: no gameplay resources or refills.
- Premium or strategic restrictions: unchanged; no premium/resource/account action.
- Active evidence manifest: completed task-local chain ending at `.local-captures/flow-delivery/CAMPAIGN-ATLAS-NATIVE-SURVEY-AND-VALIDATION/survey-20260724T023336884972Z`; `pnsctl bluestacks verify-flow` returned `verified` / `native_survey_complete`.
- Required artifacts: four edge clamps, accepted overlap associations, coverage and loop-closure reports, cross-difficulty geometry, landmark annotations, exact accounting, and a recognized safe terminal.
- Immediate-before/immediate-post/result/journal: Required for every dispatched navigation input.
- Additional task-specific artifacts: selection-aware tier1/tier2 selected/unselected templates plus ground-truth annotations, and unhighlighted campaign-exit template/ground-truth, under `tasks/assets/campaign_auto_battle/800x1280/`. Completed live session evidence is retained under `.local-captures/flow-delivery/CAMPAIGN-ATLAS-NATIVE-SURVEY-AND-VALIDATION/survey-20260724T023336884972Z`.
- Focused tests: `tests/test_campaign_atlas.py`, `tests/test_campaign_atlas_vision.py`, `tests/test_campaign_atlas_collector.py`, `tests/test_flow_delivery_authority_consistency.py`, `tests/test_gameplay_flow_contracts.py`, `tests/test_flow_delivery_orchestrator.py`, `tests/test_home_atlas_verified_route.py`, and `tests/test_governance_validation.py`.
...
- Known baseline failures: full discovery before the final queue transition ran 1,357 tests with 13 state-dependent failures and 2 skips; 1,342 passed. Focused Campaign, authority, governance, Home-route, and popup suites passed 152 tests. The failures were checked-in queue-state expectations while this flow was still active; no Campaign runtime, vision, collector, or evidence-verifier failure remained.
- Evidence requirement: NOT_APPLICABLE for the canonical governance-manifest slot; this navigation-validation flow uses its task-local retained report and `pnsctl` verifier. The independently accepted two-hash chain ending at `survey-20260724T023336884972Z` must not be fabricated, overwritten, or deleted.
...
- Current authorization: terminal. The single live attempt completed at 93 inputs (AUX7/EDGE24/OVERLAP62), runtime ownership was released, the lease was reconciled, and no further input is authorized by this completed task.
...
- Commit policy: one focused conventional local commit; no push.
- Expected focused commits: completed as `1e58f66 feat(campaign): validate native atlas survey` and `d50d7c8 chore(flow-delivery): close campaign survey flow`.
- Completion boundary: independently reviewed hash-bound corpus and atlas validation only; no gameplay execution promotion.
- Completion criteria: independently reviewed hash-bound corpus validates four edges, overlapping coverage, loop closure, Story 1/2 geometry, required landmarks, exact accounting, and a safe terminal; consumers/registration/scheduler remain blocked; required tests and diff check pass; one focused commit is created.
...
- Status: end-to-end flow `LIVE_VALIDATED` (2026-08-17; the challenge was completed in-game and
  return-to-Home was proven live via the measured transparent campaign-exit control). Shipped in
  commits `3951c4b` and `431534e`.
...
- Live proof: the prior unsupported-Home-zoom block is resolved. Return-to-Home was proven live via
  the measured transparent campaign-exit control (ROI ~[690,920,800,1060], `campaign-exit-base`)
  added by the `--campaign-exit-home-only` mode in `scripts/bluestacks_ultimate_challenge.py`, with
  `recognize_exit_dialog` hardening in `tasks/troop_training_vision.py`. The challenge was completed
  in-game and `complete_for_reset` persisted for reset identity `game-day-2026-08-17`.
- Reset record: `.local-captures/ultimate-challenge-daily-reset-window.json`
  (`completion_state: completed`, `last_success_at: 2026-08-18T02:28:34Z`,
  `last_success_reset_identity: game-day-2026-08-17`).
...
- Completion evidence: reset record
  `.local-captures/ultimate-challenge-daily-reset-window.json` records `complete_for_reset` for
  `game-day-2026-08-17`; reproof manifest `docs/ultimate-challenge-lean-reproof-manifest-r1.md`;
  live return-to-Home proof and de-duplicated focused tests shipped in `3951c4b`/`431534e`.
...
- Status: Completed (2026-07-22; candidate `c3a4b3a` completed canonical Home → Research Lab →
  Nova → Home with four navigation inputs, verified terminal Home, and zero Praise).
- Milestone: Gameplay Flow Factory Milestone A.
...
- Blocked by: None. Earlier `initial_radial_missing_research_lab_provenance` and wrong-template
  sessions remain historical evidence and do not describe the accepted terminal run.
...
- Established facts: pre-input candidate `58f7343` recognized the current screen as Nova and sent
  ...
  Follow-up corrections `510e73a` and `c3a4b3a` bound the actual Nova portrait and accepted measured
  Home zoom context. Session `nova-navigation-canary-20260722T020656687010Z` then completed the
  canonical route with `navigation_input_count=4`, `praise_taps=0`, and
...
- Transitive regression set: Nova navigation/vision, pnsctl/scenario, Home route, governance/context,
  and full suite.
- Allowed changes: only the offline template-radial correction, synchronized attempt authority,
  task-local tests/assets, and exact governance records listed below.
- Per-commit allowed paths: `scripts/nova_praise_bluestacks.py`,
  `tasks/nova_praise_vision.py`, `tasks/assets/nova_praise/800x1280/**`,
...
- Prohibited changes: no further no-Praise canary retry, evidence deletion, production registration
  or scheduling, plan edits, or push. Any consequential validation requires a separate promoted
  task and explicit product policy.
- Authorized runtime action: completed; no further GF-MVP-009 runtime input is authorized.
- Maximum transport inputs: Terminal accepted execution used four navigation inputs and zero
...
- Actions that must not be repeated: any GF-MVP-009 canary execution; the accepted terminal route is
  not retry authority for Praise or another navigation run.
- Required source: prior terminal session
  ...
  accepted terminal session
  `.local-captures/flow-delivery/NOVA-PRAISE-HOME-ATLAS-MIGRATION/nova-navigation-canary-20260722T020656687010Z`,
...
- Ambiguous-result behavior: historical blocks remain retained; the accepted terminal result closes
  this task without authorizing a retry.
- Zero-cost requirement: no game resource/currency; zero Praise.
- Quantity limits: named scenario maximum 2 / count 2 / exhausted; flow-level
  maximum_live_attempts 2 / live_attempt_count 2. Pre-input results remain separate and do not
  consume or reopen execution budget.
- Resource consumption policy: none.
...
- Valid blocked outcomes: historical blocks remain valid evidence but are superseded as task state
  by the accepted completed route.
- Terminal-result commit policy: preserve all sessions, prohibit canary retries, keep unresolved
  clear and registration disabled.
...
- Completion criteria: Met; canonical Home → Research Lab → Nova → Home completed with four
  navigation inputs, zero Praise, verified terminal Home, retained evidence, and disabled
...
- Status: Ready but not active; the explicitly authorized supervised Praise validation is the
  current atomic task.
- Scope: convert only the accepted completed GF-MVP-009 live evidence in a later atomic task.
- Authority: accepted plan; historical blocked evidence must not be converted into successful proof.
...
- Full-list chest follow-up (2026-08-13): the production chest-only route now restores and audits each current-frame viewport, scrolls through the complete 12-identity list, and binds the bottom Cube row independently. Gear (`+496`), Glory (`+408`), Bioenhancer (`+11`), Ultimate (`+784`), and Chip (`+488`) were claimed exactly once, taking the follow-up balance `14462 → 16649` (`+2187`). The final coverage is Hero/Weapon/Tech/Gear/Nova/Module/Glory/Bioenhancer/Ultimate/Chip already claimed and Core/Cube locked, with no eligible, clipped, unobserved, or unresolved row. Terminal development session `.local-captures/development-sessions/RUINS-CHALLENGE-HOME-ATLAS-MIGRATION-20260813T062938293481Z` and runtime session `.local-captures/flow-delivery/RUINS-CHALLENGE-HOME-ATLAS-MIGRATION/nav-20260813T062938638407Z/ruins-challenge-20260813T062939116724Z` prove canonical Home and released ownership. Claim evidence is retained under `nav-20260813T055939375022Z`, `nav-20260813T061151633857Z`, and `nav-20260813T062244588104Z`. Focused validation passed 147 tests; no combat, Ruins-point spend, Exchange, Mall, purchase, registration, scheduler, composition, M6, or Bliss change occurred.
