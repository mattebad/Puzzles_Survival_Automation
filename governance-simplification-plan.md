...
Every scheduled occurrence has a deterministic key. Cardinality is route-specific: daily-once occurrences use the stable flow ID and reset ID exactly once per reset; reset-bounded Claim occurrences use the stable flow ID, reset ID, durable canonical ready-batch identity, ordinal, and `revision_within_reset`, allowing distinct later ready batches within one reset; repeated-timer and generation-fence contracts remain unchanged (timer occurrences use the stable flow ID and persisted schedule anchor/cadence slot, while other bounded-repeat occurrences retain their persisted sequence/anchor). The canonical batch identity is persisted before claim, immutable on the run, and reused by retry and restart. A new capture hash, OCR rescan, or UI revisit never mints a batch; changed UI counts only after state is reconciled and a genuinely new ready batch exists. `UNKNOWN` never rolls a batch forward to retry. A restart or reset crossing cannot create a second key for the same occurrence or mutate the identity of a run already in progress. Manual runs use a persisted operator request key and still pass the same enable, lease, and action gates.
