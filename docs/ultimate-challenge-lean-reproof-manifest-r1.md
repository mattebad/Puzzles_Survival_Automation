...
## Refrozen stage r16 — state-specific Hero Lineup overlay binding
- User authorization: autonomous redesign around non-absolute blockers remains active. The r15 canary advanced through five accounted inputs to a stable Hero Lineup and exposed a state-specific recognizer defect.
- Revision ID: `ultimate-challenge-lean-reproof-r16`.
- Failure class: `local_defect`.
- Evidence:
  - the red Challenge target and tap were correct; six post captures over 8.95 seconds are byte-identical stable Hero Lineup;
  - title OCR independently reads ordered `Hero Lineup` tokens at confidence 96;
  - one unique gold Challenge component and all five selected-card signatures satisfy existing geometry;
  - generic popup detection reports one nested contour cluster generated by the five-card grid, causing false denial;
  - authoritative child runtime count is `5/16`; all five actions have semantic successor evidence, ownership released, and no unresolved global gate remains.
- Writable paths:
  - `scripts/bluestacks_ultimate_challenge.py`
  - `tests/test_bluestacks_ultimate_challenge.py`
- Add a Hero-Lineup-only binder contract requiring all three independent signals: ordered title tokens `Hero` then `Lineup` from the native title ROI at confidence at least 80, exactly one gold bottom Challenge control, and all five selected-card signatures.
- In that state only, ignore central popup candidates when they match the measured nested card-grid envelope and cover at least 78% of the independently bound selected-card union. Any independent centered, shifted, broad, purchase, or card-obscuring panel must deny. Do not weaken `_unexpected_visual_popup` for any other state.
