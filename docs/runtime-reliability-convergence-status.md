...
- Initial Terra review found one residual full-frame digest comparison in the semantic disappearance postcondition. The parent accepted it as `local_defect`; the single consolidated correction removed digest equality from that decision and added an equal-recognition-digest successor regression. Terra's corrected recheck passed.
