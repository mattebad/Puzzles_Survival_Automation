...
## Stage 10 Phase 5 startup-surface recovery r2 acceptance
- Original frozen implementation: `docs/execution-manifests/runtime-reliability-stage-10-phase-5-startup-surface-recovery-r1.md`. It remains immutable and contains no post-freeze receipt chronology.
- Accepted integration repair: `docs/execution-manifests/runtime-reliability-stage-10-phase-5-startup-surface-recovery-r2.md`, implementation commit `c23f9da`. R2 supersedes r1 for integration only. The outer-summary failure was classified `local_defect`.
- Attempt 1 used the misbound ROI `(11,54,72,117)` and produced no observed effect. A separately authorized materially changed attempt used the corrected visible in-game Back ROI `(39,0,168,61)` at `(103,30)`, dismissed Scarlett, and retained canonical Home with correlation approximately `0.9849`.
- Successful recovery session: `.local-captures/development-sessions/AUTONOMY-SERVICE-CAMPAIGN-NAVIGATION-PROVING-SLICE-20260826T205944685287Z`. Settled Home observation: `.local-captures/development-sessions/observe-20260826T210014287650Z`.
- Child and outer accounting now agree: recovery `1`, route `0`, total `1`, outer `input_count=1`. The outer terminal is `completed` with `completion_scope=startup_recovery_only`; the route runner is not called, the post-recovery typed observation is retained, ownership is released, no budget is reopened, and identical retry remains denied.
- Full-frame hashes are retained only as evidence provenance/invocation metadata. Scarlett recognition and revalidation use stable current-frame title, safe-Back, and surface-identity ROIs. The animation-variance positive remains covered.
- Initial Terra review found one residual full-frame digest comparison in the semantic disappearance postcondition. The parent accepted it as `local_defect` and removed digest equality from that decision. Terra's single recheck confirmed the production correction but returned `NEEDS_FIX` because the regression equalized only recognition metadata, not the retained capture digest. Commit `8b8e372` strengthens the test so the post capture reuses the probe payload/SHA while semantics report Scarlett absent and Home admissible; the exact regression and all 18 startup tests pass. No second recheck was run because the r2 budget allowed only one.
- Final offline receipts: exact outer regression 1/1; startup recovery 18/18; DevelopmentSession 26/26; affected Campaign/Recruitment routes 40/40; shared-navigation 20/20 with receipt `19df01c8a318d2e92e1fb3a8f076a50e27050a6c6e1891359e275958cab751e1`; governance passed; `git diff --check` passed. Full unittest discovery was not run.
- Registration remains `NOT_REGISTERED`; scheduler eligibility remains disabled. No purchase, Confirm, real-money, Android Back, route, Campaign, or other gameplay input followed the successful recovery.
- Stage B is `not_applicable` for this occurrence because the settled successor was canonical Home. Separate startup shop page and modal variants remain `evidence_required` until a natural native occurrence; they were not manufactured and are not claimed implemented.
- Parent integration decision: `accepted for Stage A r2`. Campaign AP r2 may begin only after this closure is committed and pushed.

## Stage 10 Phase 5 Campaign AP r2 closure
- Frozen revision: `docs/execution-manifests/runtime-reliability-stage-10-phase-5-campaign-ap-r2.md`, entered from pushed Stage A closure `0fe5186ea9211922c34f59bf3617f4e00177ae70`. The retained r1 `blocked_evidence_required` history is unchanged.
- Zero-input preflight `.local-captures/development-sessions/observe-20260826T214533950614Z` captured native `800x1280` canonical Home with correlation `0.9849079251289368`, visible Campaign AP `120/120`, no refill surface, zero input, no lifecycle state, and released ownership.
- One isolated offline AP-regeneration pulse used fresh observed balance `120`, selected only `CAMPAIGN-AP-AUTO-BATTLE-LIVE-CANARY`, bound product `campaign_ap-v1` and the exact fixed registration, and reported `transport_count=0`.
- The sole r2 occurrence is `.local-captures/development-sessions/CAMPAIGN-AP-AUTO-BATTLE-LIVE-CANARY-20260826T214745064655Z`. It atomically consumed the exact registration before runtime. The existing route then returned `LOCALIZATION_NOT_RECOGNIZED` during canonical Home Atlas localization and failed closed before any input.
- Terminal truth: recovery input `0`, route input `0`, total input `0`, Campaign action count `0`, AP spend `0`, no refill/item/premium/Sweep/Blitz/Auto Complete/stamina/march/PvP/real-money action, no unresolved effect, and ownership released. Final production registration is `NOT_REGISTERED` and scheduler eligibility is disabled.
- Parent classification is `local_defect`; disposition is `blocked_evidence_required`. The misleading outer fallback reason `Campaign AP ledger does not equal configured cost` does not override the retained child reason `LOCALIZATION_NOT_RECOGNIZED`; because no input occurred, r2 closes without reconciliation or retry.
- No identical retry, post-block repair, additional live observation, or further gameplay input is authorized. PR #4 remains open and unmerged; external approval remains the final gate.