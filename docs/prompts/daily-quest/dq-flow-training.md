...
Policy: DISABLED_POLICY; no resource transaction, dispatch, or promotion. Offline queue contract
is implemented in `tasks/training_disabled.py` for Fighter, Rider, Shooter, and Vehicle; dispatch
remains unconditionally blocked. Postcondition: offline contract proves no live action. Recovery:
fail closed on ambiguous unit, cost, queue, or successor. Daily reconciliation preserves four keys;
Claim independent. Persistence/scheduler dormant.

Tests: `tests/test_training_disabled.py` covers variant separation, no duplicate ownership,
disabled-flow validator, no registry entry, scheduler false, offline queue model, Main negative,
and Claim separation. Bliss/GnBots evidence
