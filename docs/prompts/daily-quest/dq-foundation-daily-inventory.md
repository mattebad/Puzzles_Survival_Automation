...
Scope: selected Daily tab and complete inventory binding, including current rows, aliases,
quantities, Vehicle Depot wording, Gather Food/Gathered Food, Ultimate Challenge, Hunt Zombie,
Own Hero, and Headquarters attack/win. Reusable components: `tasks/daily_quest.py`, profile
anchors, safe navigation, catalog loader. Route: home → quest → selected Daily tab.
...
Daily reconciliation: map rows through catalog aliases without merging semantics. Claim behavior:
